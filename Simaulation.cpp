...
{
    int y=transactionsDejour.size();
    Transaction t("",0,RIEN);
    if(y==1) {return t;}
     else{
    srand(time(0));
    int r=rand()%(transactionsDejour.size()-1);
...
void Simulation::simulerUnJour()//aujourd'hui
{
    ...
    {   cout<<"\n                                     \n"<<endl;
        ...
        if (i.type==ACHAT)
        {
            ...
            if(b->chercherAction(dateAujour,i.action)==1)
            {
                double s;
...
*/
        ...
        if (i.type==VENTE )
        {
            ...
            {
                cout<<"l' action que vous desirez vendre: "<<i.action<<endl<<"a une quantite= "<<i.qtite<<endl;
                cout<<"Transaction echouee!!"<<endl;
                cout<<"car cette action n'existe pas aujourd'hui donnez donc une autre decision"<<endl;
                /*i=trd->decider(*b,port);
                if(i.type==RIEN )
                {
                    cout<<"votre choix est rien faire "<<endl;
